## Team Members
##- Steve Cho (USC ID: 4314516349)
##- Sivan Nir (USC ID: 7594069996)

import csv
import matplotlib.pyplot as plt

#read csv
def load_csv(filepath="output/data.csv"):
    timestamps = []
    left       = []
    right      = []
    avg        = []
    with open(filepath) as f:
        reader = csv.DictReader(f)
        for row in reader:
            timestamps.append(float(row["timestamp"]))
            left.append(float(row["left"]))
            right.append(float(row["right"]))
            avg.append(float(row["average"]))
    return timestamps, left, right, avg

#generate figure
def make_chart(timestamps, values, title, color):
    """Generate a single dark-themed figure."""
    fig, ax = plt.subplots(figsize=(2, 1.5))
    ax.plot(timestamps, values, color=color)
    ax.set_title(title, color="white", fontsize=7)
    ax.set_xlabel("Time [s]", color="white", fontsize=6)
    ax.set_ylabel("ADC Value", color="white", fontsize=6)
    ax.tick_params(colors="white", labelsize=5)
    for spine in ax.spines.values():
        spine.set_edgecolor("white")
    ax.set_facecolor("#1e1e1e")
    fig.patch.set_facecolor("#1e1e1e")
    fig.tight_layout()
    return fig

#return 3 figures and data arrays
def generate_graphs(filepath="output/data.csv"):
    """
    Read CSV and return 3 figures + the 3 data arrays.
    Call this from app.py.
    """
    timestamps, left, right, avg = load_csv(filepath)
    fig1 = make_chart(timestamps, left,  "Left Sensor (J2)",  "steelblue")
    fig2 = make_chart(timestamps, right, "Right Sensor (J3)", "coral")
    fig3 = make_chart(timestamps, avg,   "Average Intensity", "mediumseagreen")
    return fig1, fig2, fig3, left, right, avg
