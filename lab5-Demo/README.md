...
Answer: If we increase UDP bandwidth using -b 100M, the sender attempts to transmit data at a higher rate, which can exceed the capacity of the wireless network or the signal strength cannot compensate that rate. Thus, this could lead to packet losses. UDP does not automatically lower the sending rate, so increasing bandwidth leads to signficantly higher packet loss and would not necessarily increase throughput. 
